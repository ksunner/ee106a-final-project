...
class ConstantTransformPublisher(Node):
    def __init__(self):
        ...
        self.transform.transform.translation.x = float(G_ar_base[0, 3])
        self.transform.transform.translation.y = float(G_ar_base[1, 3])
        self.transform.transform.translation.z = float(G_ar_base[2, 3])
        r = R.from_matrix(G_ar_base[:3, :3])
        q = r.as_quat()
